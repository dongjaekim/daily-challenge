...
export function CommentList({ 
  ...
  const formatComment = (apiComment: any): IComment => {
    return {
      id: apiComment.id,
  ...
  useEffect(() => {
    if (initialComments) {
      const formattedComments = initialComments.map(comment => formatComment(comment));
  ...
  const fetchComments = async () => {
    ...
    try {
      ...
      const formattedComments = data.map((comment: any) => formatComment(comment));
      setComments(formattedComments)
    } catch (error) {
      toast({
        title: '오류',
  ...
  const handleCommentUpdated = (updatedComment: any) => {
    ...
    if (isRootComment) {
      // 최상위 댓글이면 직접 업데이트
      setComments(prev => 
        prev.map(c => c.id === formattedComment.id ? { ...c, ...formattedComment } : c)
      )
    } else {
