...
import { Edit, Loader2, MessageSquare, MoreVertical, Reply, Trash, User } from 'lucide-react'
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
// Comment 컴포넌트에서 사용하는 표준 IComment 타입
...
interface CommentProps {
  ...
  postId: string
  onReplySubmit: (comment: IComment) => void
...
export function Comment({
  ...
  const isDeleted = comment.is_deleted || comment.content === '삭제된 댓글입니다.'
  ...
  const formatComment = useCallback((apiComment: any): IComment => {
    return {
      id: apiComment.id,
      content: apiComment.content,
      created_at: apiComment.created_at || new Date().toISOString(),
      updated_at: apiComment.updated_at || apiComment.created_at || new Date().toISOString(),
      ...
      author: {
        ...
        clerkId: apiComment.author?.clerkId
      },
  ...
  return (
    <div className={`py-4 ${comment.parent_id ? 'pl-8 border-l' : 'border-b'} relative`}>
        ...
        <div className="flex-1">
          <div className="flex items-center gap-2 justify-between">
            <div className="flex items-center gap-2">
              <span className="font-medium">{comment.author?.name || '알 수 없는 사용자'}</span>
          ...
          {isEditing ? (
            ...
            <p className={`mt-1 ${isDeleted ? 'text-muted-foreground italic' : ''}`}>
