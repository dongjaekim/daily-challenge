...
export async function POST(
  ...
  try {
    ...
    if (parentId) {
      const parentComment = await supabaseDb.select('post_comments', { id: parentId })
      ...
      if (parentComment[0].parent_id) {
        return new NextResponse('Maximum comment depth reached (2)', { status: 400 })
      }
    }
  ...
  try {
    ...
    const comments = commentsData?.map(comment => {
      const author = comment.users ? {
        ...comment.users,
        avatar_url: comment.users.avatar_url || `https://api.dicebear.com/7.x/initials/svg?seed=${encodeURIComponent(comment.users.name || 'User')}`
      } : null
    ...
    const hierarchy = rootComments.map(rootComment => {
      const replies = replyComments.filter(reply => reply.parent_id === rootComment.id)
      return {
        ...rootComment,
        replies
      }
