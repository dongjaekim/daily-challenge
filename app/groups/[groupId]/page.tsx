...
export default async function GroupPage({
  ...
  if (!memberArr.length) {
    ...
    memberArr.push({ group_id: params.groupId, user_id: uuid, role: "member" });
  }
    ...
    <div className="space-y-8 md:space-y-10">
      ...
      <Tabs defaultValue={currentTab} className="w-full">
        <TabsList className="grid w-full h-full grid-cols-3 md:w-auto md:min-w-[400px] mx-auto shadow-sm">
          ...
          >
            <Link
              href={`/groups/${params.groupId}?tab=calendar`}
          ...
          >
            <Link
              href={`/groups/${params.groupId}?tab=stats`}
              className="flex items-center gap-1.5 w-full justify-center py-2"
            >
        ...
        <TabsContent value="calendar" className="mt-6 md:mt-8">
          <GroupCalendarView groupId={params.groupId} challenges={challenges} />
        </TabsContent>

        <TabsContent value="posts" className="mt-6 md:mt-8">
          <GroupPostsView groupId={params.groupId} challenges={challenges} />
        </TabsContent>

        <TabsContent value="stats" className="mt-6 md:mt-8">
          <GroupStatsView groupId={params.groupId} challenges={challenges} />
        </TabsContent>
      </Tabs>
    </div>
  );
