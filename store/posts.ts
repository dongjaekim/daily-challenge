...
interface PostsState {
  ...
  // 특정 그룹의 게시글 설정
  setPosts: (groupId: string, posts: IPost[], challengeId?: string | null) => void
  ...
  // 특정 그룹의 게시글 업데이트
  updatePost: (groupId: string, postId: string, updatedPost: Partial<IPost>) => void
  ...
  // 댓글 수 업데이트
  updateCommentCount: (groupId: string, postId: string, isIncrement: boolean) => void
          ...
          [groupId]: {
            ...
            posts: groupData.posts.map(post => {
              if (post.id === postId) {
                ...
                return {
                  ...post,
                  commentCount: isIncrement ? commentCount + 1 : Math.max(0, commentCount - 1),
                  comments: isIncrement ? (post.comments || 0) + 1 : Math.max(0, (post.comments || 0) - 1)
                }
              }
              return post
            })
